...
<script>
	$(function() {
		...
		$('#gridblockModal').on('show.bs.modal', function(e) {
			$('html').addClass('modal-opened');
		})
		$('#gridblockModal').on('hide.bs.modal', function(e) {
	...
	function gridblock_setGridSortedSlices(colID) {
		...
		if (colID > 0) {
			...
			$('#gridblockColumnWrapper' + colID + ' .column-slice-functions input[type=hidden]').each(function() {
				uID = $(this).data('uid');
				if (uID != undefined && uID != "" && parseInt($(this).val()) > 0) {
					slices.push(uID);
				}
			...
			if (i <= cols) {
				...
				<?php if ($config['previewtabnames'] == 'checked') : ?>
					...
					if (tabname != undefined && tabname != "") {
						$('a[href="#gridblockCol' + colID + '"]').text(tabname);
					}
				<?php endif; ?>
	...
	function gridblock_showGridTemplate(id) {
		...
		if (id > 0) {
			...
			if (html != undefined && html.length > 10) {
				$('div#gridblock-template').html(capt + html);
			}
	...
	function gridblock_showHideContentSettings(datalen, templateID, colID = 0) {
		...
		if (datalen > 0 && templateID > 0) {
			if (colID == 0) {
				$('.gridblock-top a.btn-options').css('display', 'inline-flex');
			}
			if (colID > 0) {
				$('#gridblockColumnWrapper' + colID + ' div.optionstoggler').show();
			}
		} else {
			if (colID == 0) {
				$('.gridblock-top a.btn-options, .gridblock-top .gridblockoptions').hide();
			}
			if (colID > 0) {
				$('#gridblockColumnWrapper' + colID + ' div.optionstoggler, #gridblockColumnWrapper' + colID + ' .gridcolumnoptions').hide();
			}
		}
	}
	...
	function gridblock_checkDeleteButtons(colID) {
		...
		if (colID > 0) {
			...
			if (dst.length < 2) {
				dst.addClass('column-slice-nodelete');
			} else {
				dst.removeClass('column-slice-nodelete');
			}
			count = dst.length;
		}

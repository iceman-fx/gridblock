...
class rex_article_content_gridblock extends rex_article_content_editor
{
	...
	public function getModuleEdit($addModuleID = 0, $colID = 0, $uID, $rexVars = array(), $action = "")
	{
		...
		if ($MOD->getRows() != 1) :
			...
			$moduleInput = $MOD->getValue('input');
			$moduleInput = rex_extension::registerPoint(new rex_extension_point(
				'GRIDBLOCK_CONTENT_MODULE_INPUT',
				$moduleInput,
				[
			...
			$moduleInput = rex_gridblock_var_replacer::replaceModuleVars($moduleInput, $rexVars);								//zuerst die RexVars ersetzen
	...
	public function getModuleOutput($moduleID, $uID, $rexVars = array())
	{
		...
		if ($MOD->getRows() != 1) :
			...
			//Variablen ersetzen
			$moduleOutput = $MOD->getValue('output');

			$moduleOutput = rex_extension::registerPoint(new rex_extension_point(
				'GRIDBLOCK_CONTENT_MODULE_OUTPUT',
				$moduleOutput,
				[
					'module_id' => $moduleID,
				]

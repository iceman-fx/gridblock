...
class rex_article_content_gridblock extends rex_article_content_editor {
	...
	{	//if (!rex::getUser()) { self::deleteCookie(); }         //Cookie im Frontend löschen -> setzt leider das Cookie ohne es korrekt zu löschen
	}
	

    //public function getModuleEdit($addModuleID = 0, $colID = 0, $uID, $rexVars = array(), $copyID = "", $copyCOL = 0, $copyCOL = 0, $copySLID = 0)
	public function getModuleEdit($addModuleID = 0, $colID = 0, $uID = 0, $rexVars = array(), $action = "")
	...
	{	$options = array(
        ...
        setcookie(self::getCookieName(), '', $options);
	...
	{	$value = (!is_array($value)) ? array('value' => $value) : $value;
        ...
        setcookie(self::getCookieName(), json_encode($value), $options);
    }
